...
def process_mode(mode):
    ...
    df_interval_accuracies.to_csv(f'plots/WP_dev/interval_accuracies_{mode}.csv', index=False)
    print(f"Interval accuracies for {mode} learning saved to 'interval_accuracies_{mode}.csv'.")

    # Save detailed results to CSV
    df_results = pd.DataFrame(all_results)
    df_results.to_csv(f'plots/WP_dev/interval_predictions_{mode}.csv', index=False)
